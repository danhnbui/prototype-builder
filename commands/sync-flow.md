...
- **Wireflow nodes (v0.3.15+)** — every screen-shaped node label MUST match a `PB_DATA.handoff.screens[N].name` from Tab 4. Each `WIREFLOW_SCREENS` entry carries `{ fn, label, status, preview }` where `status` is one of `NEW · IN PROGRESS · DONE · ATTENTION · ASAP · REVIEW · PAUSE` and `preview` is one of `form-1 · form-2 · form-3 · otp · success · block`. The template's `wfCardHtml()` + `wfMiniPreview()` helpers render each node as a real **card** (title + colored status badge + stylized mini-mockup of the screen) embedded inside the Mermaid HTML label. `WIREFLOW_NOTES` (note number → node id) wires up the amber numbered badges that cross-reference the `<ol class="flow-doc-notes">` panel in the sidebar above the test checklist. Use `buildWireflowMermaid()` to assemble the Mermaid source rather than writing the HTML labels by hand.
...
2. **Replace the body of `renderFlowPopulated()`** with the new `.flow-doc-grid` structure containing:
   ...
   - `<section class="flow-doc-main">` — header with title + actor/entry/goal sub + the legend pill button (`onclick="openLegendPopover(this)"`), then `<div class="flow-canvas" id="flow-canvas">` containing `<div class="flow-viewport" id="flow-viewport"><div class="flow-stage" id="flow-stage">…</div></div>` plus the `flow-zoom-controls` (+ / − / ⊙), `flow-zoom-readout`, and `flow-hint`. Inside `#flow-stage`, place **one `<div class="flow-doc-section">`** with the combined flow (heading + intent + `<div class="mermaid">…</div>`). Only add more sections when truly necessary per platform rule §0.4. Do NOT render summary or validation paragraphs below the canvas — the canvas, the legend popover, and the sidebar Flow notes carry everything the reader needs; run the §7 validation checklist internally as a QA gate but don't print it.
